...
FDFS_BASE_URL = 'http://192.168.70.129:8888/'
...
CRONJOBS = [
    # 每5分钟执行一次生成主页静态文件
    ('*/5 * * * *', 'contents.crons.generate_static_index_html', '>> /home/henrylee/Desktop/meiduo_mall/logs/crontab.log')
    # 不能是相对路径,如果是相对路径的话就不是Django执行而是操作系统执行 ('*/5 * * * *', 'contents.crons.generate_static_index_html', '>> '+os.path.join(os.path.dirname(BASE_DIR), '/logs/crontab.log'))
]
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL': 'http://192.168.70.129:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
